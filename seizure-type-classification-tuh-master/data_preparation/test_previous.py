...
def generate_data_dict(xlsx_file_name, sheet_name, tuh_eeg_szr_ver):
    ...
    print('\n\nWARNING : train_files is restricted (not run on all data)\n\n')

    for item in train_files[:20]:
        a = item[0].split('/')
        if tuh_eeg_szr_ver == 'v1.5.2':
            patient_id = a[4]
        else:
...
def gen_raw_seizure_pkl(args, tuh_eeg_szr_ver, anno_file):
    ...
    seizure_types = ['BG']+args.seizure_types # Add the background type
    ...
    if merge_train_dev : folders = ['merged']
    else : folders = ['dev','train']
    ...
    if tuh_eeg_szr_ver == 'v1.5.2' :
        if merge_train_dev :
            ...
            seizure_data_dict = load_edf_extract_seizures_v152(raw_data_base_dir, os.path.join(save_data_dir, 'merged'), merged_dict)
            print('\n\n')
        else :
