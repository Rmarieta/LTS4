...
# python .\data_preparation\test_extract_bg_samples.py
...
def generate_data_dict(xlsx_file_name, sheet_name, seizure_types):
    ...
    ref_txt = pd.read_csv("../data/v1.5.2/_DOCS/ref_"+sheet_name+".txt", sep=" ", header=None)
    ...
    for seizure_type, seizures in data_dict.items():
        ...
        for seizure in seizures:
            ...
            try:
                ...
                with open(os.path.join(seizure_data_dir, 'szr_' + str(count) + '_pid_' + patient_id + '_type_' + seizure_type + '.pkl'), 'wb') as fseiz:
...
def gen_raw_seizure_pkl(args, tuh_eeg_szr_ver, anno_file):
    ...
    save_data_dir = os.path.join(base_dir, tuh_eeg_szr_ver, 'input_test')
    ...
    train_data_dict = generate_data_dict(szr_annotation_file, 'train', seizure_types)
    ...
    dev_test_data_dict = generate_data_dict(szr_annotation_file, dev_name, seizure_types)
    ...
    if tuh_eeg_szr_ver == 'v1.5.2' :
        ...
        seizure_data_dict = load_edf_extract_seizures_v152(raw_data_base_dir, os.path.join(save_data_dir, 'dev'), dev_test_data_dict)
        print('\n\n')
        print('Extracting seizures from the train directory...\n')
        seizure_data_dict = load_edf_extract_seizures_v152(raw_data_base_dir, os.path.join(save_data_dir, 'train'), train_data_dict)
        print('\n\n')
    else:
        exit('tuh_eeg_szr_ver %s is not supported' % tuh_eeg_szr_ver)
...
def main():
    ...
    parser.add_argument('--seizure_types',default=['FNSZ','GNSZ'], help='types of seizures for the classification')
