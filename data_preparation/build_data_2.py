...
def generate_data_dict(base_dir, xlsx_file_name, sheet_name, seizure_types, nb_szr, len_chunk):
    ...
    for type in seizure_types :
        ...
        idx = random.sample(range(len(type_files)), nb_szr)
        for item in type_files[idx] :
            a = item[0].split('/')
            patient_id = a[4]

            v = seizure_info(patient_id = patient_id, filename = item[0], start_time=max(0,(item[1]-len_chunk)), end_time=item[2])
...
def gen_raw_seizure_pkl(args, anno_file):
    ...
    nb_szr = args.nb_szr
    len_chunk = args.len_preceding_chunk

    # Create the directory where the raw seizure data will be extracted
    save_data_dir = os.path.join(base_dir, 'v1.5.2', 'szr_chunks')
    ...
    train_data_dict = generate_data_dict(base_dir, szr_annotation_file, 'train', seizure_types, nb_szr, len_chunk)
    ...
    dev_test_data_dict = generate_data_dict(base_dir, szr_annotation_file, 'dev', seizure_types, nb_szr, len_chunk)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nb_szr',default=10, help="number of seizures that will be saved")
    parser.add_argument('--len_preceding_chunk',default=5, help="length of the chunk to be kept before the seizure in [s]")
